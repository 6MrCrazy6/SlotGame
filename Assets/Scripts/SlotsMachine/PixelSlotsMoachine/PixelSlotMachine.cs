...
public class PixelSlotMachine : MonoBehaviour
{
    public PixelSlots[] slots;
    public PixelCombinations[] combinations;
    ...
    {
        isSpin = false;
        foreach (PixelCombinations i in combinations)
        {
            Debug.Log(slots[0].gameObject.GetComponent<PixelSlots>().stoppedSlot.ToString()); ;
            Debug.Log(i.FirstValue.ToString());
            if (slots[0].gameObject.GetComponent<PixelSlots>().stoppedSlot.ToString() == i.FirstValue.ToString()
                && slots[1].gameObject.GetComponent<PixelSlots>().stoppedSlot.ToString() == i.SecondValue.ToString()
                && slots[2].gameObject.GetComponent<PixelSlots>().stoppedSlot.ToString() == i.ThirdValue.ToString())
            {
                if (isPlayer == true && isCasino == false)
                {
                    ChangeMoney(i.prize);
